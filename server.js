...
async function attachReactions(transactions, viewerUserId) {
  ...
  return transactions.map((t) => {
    ...
    return { ...t, reaction_counts: counts, my_reaction: mine ? mine.reaction_type : null };
  });
}
...
app.post('/api/partners/edit-requests/:id/approve', verifyAuth, async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE partner_notifications SET is_read = true WHERE edit_request_id = $1`, [id]);
    await pool.query(
      `INSERT INTO partner_notifications (user_id, message, type) VALUES ($1, $2, 'info')`,
      [request.requested_by_user_id, 'আপনার এডিট এপ্রুভ করা হয়েছে ✅']
    );
...
app.post('/api/partners/edit-requests/:id/reject', verifyAuth, async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO partner_notifications (user_id, message, type) VALUES ($1, $2, 'info')`,
      [request.requested_by_user_id, 'আপনার এডিট রিজেক্ট করা হয়েছে ❌']
    );
...
// একটা নির্দিষ্ট নোটিফিকেশন রিড করুন (শুধু সেটাই সরে যাবে)
app.post('/api/notifications/:id/read', verifyAuth, async (req, res) => {
  try {
    await pool.query(`UPDATE partner_notifications SET is_read = true WHERE id = $1 AND user_id = $2`, [req.params.id, req.user.id]);
    res.json({ status: 'ok' });
  } catch (err) {
    res.status(500).json({ status: 'error', message: err.message });
  }

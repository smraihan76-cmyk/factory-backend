  ...
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      const pendingDeleteResult = await pool.query(
        `INSERT INTO order_pending_deletes (entry_id, submitted_by) VALUES ($1, $2)
         ON CONFLICT (entry_id) DO UPDATE SET submitted_by = EXCLUDED.submitted_by, submitted_at = NOW()
         RETURNING id`,
        [id, req.user.name]
      );

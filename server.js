...
app.get('/api/notifications/history', verifyAuth, async (req, res) => {
  try {
    ...
    await pool.query(
      `DELETE FROM partner_notifications WHERE is_read = true AND read_at IS NOT NULL AND read_at < NOW() - INTERVAL '24 hours'`
    );
    const result = await pool.query(
      `SELECT * FROM partner_notifications WHERE user_id = $1 AND is_read = true AND read_at >= NOW() - INTERVAL '24 hours'

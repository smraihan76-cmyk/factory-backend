...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    res.json({
      status: 'ok',
      token,
      user: { id: user.id, name: user.name, phone: user.phone, role: user.role, is_partner: user.is_partner }
    });
  } catch (err) {
    res.status(500).json({ status: 'error', message: err.message });
  }
...
app.get('/api/auth/users', verifyAdmin, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, name, phone, role, is_partner, active, created_at FROM users ORDER BY created_at DESC`
    );
...
app.get('/api/partners', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, name, phone FROM users WHERE is_partner = true AND active = true ORDER BY name ASC`
    );
...
app.post('/api/partners/transactions', verifyAuth, async (req, res) => {
  try {
    const { type, description, amount, event_time } = req.body;
    ...
    const result = await pool.query(
      `INSERT INTO partner_transactions (user_id, type, description, amount, added_by_user_id, event_time)
       VALUES ($1, $2, $3, $4, $1, COALESCE($5, NOW()))
       RETURNING *`,
      [req.user.id, type, description, amount, event_time || null]
    );

    // অন্য সব পার্টনারকে নোটিফিকেশন পাঠানো
    const otherPartners = await pool.query(
      `SELECT id FROM users WHERE is_partner = true AND active = true AND id != $1`,
      [req.user.id]
    );
    const actionLabel = type === 'expense' ? 'খরচ' : 'ক্যাশ';
    for (const p of otherPartners.rows) {
      await pool.query(
        `INSERT INTO partner_notifications (user_id, message) VALUES ($1, $2)`,
        [p.id, `${req.user.name} তার হিসাবে ৳${amount} ${actionLabel} যোগ করেছেন — "${description}"`]
      );
    }

    res.json({ status: 'ok', transaction: result.rows[0] });
  } catch (err) {
    res.status(500).json({ status: 'error', message: err.message });
  }
});
...
app.put('/api/partners/transactions/:id', verifyAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { description, amount, event_time } = req.body;
    ...
    if (existing.rows[0].added_by_user_id !== req.user.id) {
    ...
    const result = await pool.query(
      `UPDATE partner_transactions SET
       ...
       WHERE id = $4
       RETURNING *`,
      [description, amount, event_time || null, id]
    );
    ...
    const actionLabel = row.type === 'expense' ? 'খরচ' : 'ক্যাশ';
    for (const p of otherPartners.rows) {
      await pool.query(
        `INSERT INTO partner_notifications (user_id, message) VALUES ($1, $2)`,
        [p.id, `${req.user.name} তার একটা ${actionLabel} এন্ট্রি এডিট করেছেন — "${row.description}"`]
      );
    }

    res.json({ status: 'ok', transaction: row });
  } catch (err) {
    res.status(500).json({ status: 'error', message: err.message });
  }
});
...
app.get('/api/notifications', verifyAuth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM partner_notifications WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50`,
      [req.user.id]
    );
...
app.post('/api/notifications/mark-read', verifyAuth, async (req, res) => {
  try {
    await pool.query(`UPDATE partner_notifications SET is_read = true WHERE user_id = $1`, [req.user.id]);
    res.json({ status: 'ok' });
  } catch (err) {
    res.status(500).json({ status: 'error', message: err.message });
  }
...
app.delete('/api/attendance/clear-today', async (req, res) => {
  try {
    const result = await pool.query(`DELETE FROM attendance_events WHERE event_time::date = CURRENT_DATE`);
...
async function syncOneMachine(machine) {
  ...
  try {
    ...
    for (const log of newLogs) {
      ...
      const day = correctedTime.toISOString().slice(0, 10);

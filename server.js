...
app.post('/api/order-entries', verifyAuth, async (req, res) => {
  try {
    ...
    extractOrderInfoWithAI(raw_text, page_id)
      .then((extracted) => {
        pool.query(
          `UPDATE order_entries SET customer_name = $1, customer_phone = COALESCE(customer_phone, $2), customer_address = $3, amount = $4 WHERE batch_id = $5`,
          [extracted.customer_name, extracted.customer_phone, extracted.customer_address, extracted.amount, batchId]
  ...
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      if (raw_text && raw_text !== entry.raw_text && entry.status !== 'sent') {
        await pool.query(
          `UPDATE order_entries SET customer_name = NULL, customer_address = NULL, amount = NULL WHERE batch_id = $1`,
          [entry.batch_id]
        );
        extractOrderInfoWithAI(raw_text, entry.page_id)
          .then((extracted) => {
            pool.query(
              `UPDATE order_entries SET customer_name = $1, customer_address = $2, amount = $3 WHERE batch_id = $4`,
              [extracted.customer_name, extracted.customer_address, extracted.amount, entry.batch_id]
...
app.post('/api/order-entries/:id/emergency', verifyAuth, async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO order_entries (raw_text, image_urls, moderator, group_name, batch_id, page_id, page_name, status, customer_phone, customer_name, customer_address, amount, order_number)
       VALUES ($1, $2, $3, 'emergency', $4, $5, $6, $7, $8, $9, $10, $11, $12)`,
      [entry.raw_text, JSON.stringify(entry.image_urls || []), entry.moderator, entry.batch_id, entry.page_id, entry.page_name, entry.status, entry.customer_phone, entry.customer_name, entry.customer_address, entry.amount, entry.order_number]
    );

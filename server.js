...
function getPeriodRange(period) {
  ...
  switch (period) {
    ...
    case 'running_month':
      ...
      break;
    case 'last_month':
      start = new Date(Date.UTC(todayBD.getUTCFullYear(), todayBD.getUTCMonth() - 1, 1));
      end = new Date(Date.UTC(todayBD.getUTCFullYear(), todayBD.getUTCMonth(), 1));
      break;
    default:
      start = todayBD;
      end = addDays(todayBD, 1);
...
app.get('/api/order-entries/sale-summary', verifyAdmin, async (req, res) => {
  try {
    const period = req.query.period || 'running_month';

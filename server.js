...
// একই দিনে একই ইউজারের একাধিক পাঞ্চকে ক্রমানুসারে event_type-এ রূপান্তর করে
function punchesToEvents(punches) {
  const sorted = [...punches].sort((a, b) => new Date(a.time) - new Date(b.time));
  const sequence = ['check_in', 'break_start', 'break_end', 'check_out'];
  if (sorted.length === 2) {
    return [
      { event_type: 'check_in', event_time: sorted[0].time },
      { event_type: 'check_out', event_time: sorted[1].time }
    ];
  }
  if (sorted.length === 3) {
    return [
      { event_type: 'check_in', event_time: sorted[0].time },
      { event_type: 'break_start', event_time: sorted[1].time },
      { event_type: 'check_out', event_time: sorted[2].time }
    ];
  }
  return sorted.slice(0, 4).map((p, i) => ({ event_type: sequence[i], event_time: p.time }));
}
...
async function syncOneMachine(machine) {
  ...
  try {
    ...
    for (const log of newLogs) {
      ...
      if (!grouped[key]) grouped[key] = { deviceUserId: log.deviceUserId, punches: [] };
      grouped[key].punches.push({ time: log.recordTime });
    }

    let inserted = 0;
    for (const key of Object.keys(grouped)) {
      const { deviceUserId, punches } = grouped[key];
      ...
      for (const ev of events) {
        await pool.query(
          ...
          [staffId, ev.event_type, ev.event_time]
        );
        inserted++;
      }
    }

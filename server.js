...
async function attachOrderReactions(entries, viewerUserId) {
  ...
  const result = await pool.query(
    `SELECT batch_id, user_id, reaction_type FROM order_reactions WHERE batch_id = ANY($1)`,
    [batchIds]
  );
  const byBatch = {};
  for (const r of result.rows) {
...
app.post('/api/order-entries/:id/react', verifyAuth, async (req, res) => {
  try {
    ...
    const countResult = await pool.query(`SELECT COUNT(*) FROM order_reactions WHERE batch_id = $1`, [batchId]);
    res.json({
      status: 'ok',
      reaction_count: parseInt(countResult.rows[0].count),

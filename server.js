  ...
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      for (const a of admins.rows) {
        await pool.query(
          `INSERT INTO partner_notifications (user_id, message, type) VALUES ($1, $2, 'order_edit_approval')`,
          [a.id, `${req.user.name} একটা অর্ডার এডিট করতে চাচ্ছেন — অনুমোদন প্রয়োজন`]
        );
      }
...
app.delete('/api/order-entries/:id', verifyAuth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      for (const a of admins.rows) {
        await pool.query(
          `INSERT INTO partner_notifications (user_id, message, type) VALUES ($1, $2, 'order_delete_approval')`,
          [a.id, `${req.user.name} একটা অর্ডার মুছে ফেলতে চাচ্ছেন — অনুমোদন প্রয়োজন`]
        );
      }

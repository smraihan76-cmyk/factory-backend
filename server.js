  ...
  try {
    ...
    const list = staffResult.rows.map((s) => {
      ...
      return {
        ...
        check_in: s1CheckIn ? s1CheckIn.event_time : null, // ব্যাকওয়ার্ড কম্প্যাটিবিলিটি (আগের ফ্রন্টএন্ড কোডের জন্য)
...
app.get('/api/attendance/summary/:staffId', async (req, res) => {
  try {
    ...
    for (const date of Object.keys(byDate)) {
      ...
      const shift1WindowEnd = new Date(
        timeOnDate(date, duty ? duty.shift1_start : '09:00').getTime() + SHIFT_WINDOW_AFTER_MIN * 60000
      );
      const shift2WindowEnd = new Date(
        timeOnDate(date, duty ? duty.shift2_start : '15:00').getTime() + SHIFT_WINDOW_AFTER_MIN * 60000
      );

      if (s1CheckIn && s1CheckOut) {
        totalPresentMs += Math.max(0, new Date(s1CheckOut.event_time) - new Date(s1CheckIn.event_time));
      }
      if (s2CheckIn && s2CheckOut) {
        totalPresentMs += Math.max(0, new Date(s2CheckOut.event_time) - new Date(s2CheckIn.event_time));
      }
      // দুই শিফটই হাজির থাকলে মাঝের সময়টা লাঞ্চ/বিরতি হিসেবে গণনা
      if (s1CheckIn && s2CheckIn && duty) {
        const shift1End = timeOnDate(date, duty.shift1_end);
        const shift2Start = timeOnDate(date, duty.shift2_start);
  ...
  try {
    ...
    for (let d = new Date(startDate); d <= new Date(); d.setDate(d.getDate() + 1)) {
      ...
      result.push({
        ...
        shift1: { ...shift1Outcome, check_in: s1CheckIn ? s1CheckIn.event_time : null, check_out: s1CheckOut ? s1CheckOut.event_time : null },
        shift2: { ...shift2Outcome, check_in: s2CheckIn ? s2CheckIn.event_time : null, check_out: s2CheckOut ? s2CheckOut.event_time : null }
      });
    }
...
app.put('/api/staff-payments/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE staff_payments SET amount = COALESCE($1, amount), payment_date = COALESCE($2, payment_date) WHERE id = $3 RETURNING *`,
      [amount, payment_date || null, id]
    );
    if (result.rows.length === 0) {
...
async function classifyAndInsertPunches(staffId, dateStr, newPunchTimes, duty) {
  ...
  async function processShift(existingShiftEvents, newShiftPunches, shiftNum, startDt, windowEndDt) {
  ...
  await processShift(shift1Existing, shift1New, 1, shift1Start, shift1WindowEnd);
  await processShift(shift2Existing, shift2New, 2, shift2Start, shift2WindowEnd);
